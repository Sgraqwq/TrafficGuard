...
### v1.3.3 — 修复逃逸漏洞：潜水 IP 历史记录保留
- 修复计数器重置后潜水 IP（当前不活跃但有历史记录）的配额被清空的问题
- `unset` + 遗留数组遍历，确保状态文件完整保留今日所有 IP 的累计记录

### v1.3.2 — 修复 state file 写入崩溃
- 移除函数外 `local` 关键字导致 `set -e` 下脚本崩溃

### v1.3.1 — 状态文件追踪替代基线算法
- 引入独立状态文件 `daily_usage.db`，解耦 cumulative 和 raw counter
- 修复 nftables 计数器重置后累计流量丢失问题
